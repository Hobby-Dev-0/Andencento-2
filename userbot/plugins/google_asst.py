...
async def _(event):
    ...
    try:
        ...
        os.remove(required_file_name)
        await eor(event, "Processed {} ({}) in {} seconds!".format(text[:97], lan, ms))
...
CmdHelp("google_asst").add_command(
  "voice", "<reply to a msg> <lang code>", "Sends the replied msg content in audio format."
    ...
    "trt", "<lang code> <reply to msg>", "Translates the replied message to desired language code. Type '.trc' to get all the language codes", f"trt en - usero | {hl}trt en <reply to msg>"
).add_command(
