...
async def get_users(event):
    ...
    if he_ll in ["@Andencento _Chat", "@userbot_chat", "@HELLBOT_CHAT"]:
        return await user.edit("Restricted to invite users from there.")
    kraken = await get_chatinfo(event)
    chat = await event.get_chat()
    if event.is_private:
        return await user.edit("`Sorry, Cant add users here`")
    s = 0
    f = 0
    error = "None"

    await user.edit("**INVITING USERS !!**")
