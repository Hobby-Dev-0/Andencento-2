...
async def parse_ocr_space_api(event):
    ...
    try:
        ...
        await eod(user, "**Errors !!** \n`{}`\n**Report This to** {}\n\n`{}`".format(
                str(e), user_grp, json.dumps(test_file, sort_keys=True, indent=4)
            )
        )
    else:
        await user.edit("Read Document in {} seconds. \n{}".format(
                ProcessingTimeInMilliseconds, ParsedText
            )
        )
    os.remove(downloaded_file_name)
    await user.edit(ParsedText)
