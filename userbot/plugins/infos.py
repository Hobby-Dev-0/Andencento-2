...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    user = await eor(event, "recognizeing this media")
...
async def info(event):
    if event.fwd_from:
        return
    user = await eor(event, "`Analysing the chat...`")
    chat = await get_chatinfo(event)
    caption = await fetch_info(chat, event)
    try:
        await user.edit(caption, parse_mode="html")
    except Exception as e:
        print("Exception:", e)
        await eod(user, "`An unexpected error has occurred.`")
    return
...
async def _(event):
    if event.fwd_from:
        return
    user = await eor(event, "Fetching Ids...")
    if event.reply_to_msg_id:
        ...
        await user.edit("🔸 **Current Chat ID:** `{}`".format(str(event.chat_id)))
