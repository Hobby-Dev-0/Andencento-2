...
from . import *
from userbot.sql.mute_sql import is_muted, mute, unmute
...
async def promote(promt):
    ...
    try:
        ...
        await userevent.edit(f"**🔥 Promoted  [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{promt.chat.title}`!! \n**Admin Tag :**  `{rank}`")
...
async def demote(dmod):
    ...
    try:
        ...
        await userevent.edit(NO_PERM)
        return
    await userevent.edit(f"**😪 Demoted  [{user.first_name}](tg://user?id={user.id})  Successfully In**  `{dmod.chat.title}`")
...
async def muth(user):
    if user.is_private:
        ...
        replied_user = await user.client(GetFullUserRequest(user.chat_id))
...
async def ban(bon):
    ...
    try:
        ...
        await userevent.edit(f"**Banned  [{user.first_name}](tg://user?id={user.id})  in** `[{bon.chat.title}]` !!\n\nMessage Nuking : **False**")
        return
    if reason:
        await userevent.edit(f"**Bitch** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{bon.chat.title}]` !!\n**Reason :** `{reason}`")
    else:
        await userevent.edit(f"**Bitch** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{bon.chat.title}]`!!")
...
async def nothanos(unbon):
    ...
    try:
        ...
        await userevent.edit(f"[{user.first_name}](tg://user?id={user.id}) **Is Now Unbanned in**  `{unbon.chat.title}` !!")
...
async def kick(usr):
    ...
    if reason:
        ...
        await userevent.edit(f"**🏃 Kicked**  [{user.first_name}](tg://user?id={user.id})'s **Butt from** `{usr.chat.title}!`")
