...
async def _(event):
    ...
    if reply_message.sender.bot:
        ...
        return
    userevent = await eor(event, " `Scanning This media..... wait👀`")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await eod(userevent, "`Please unblock `@DrWebBot `and try again`")
            return
...
async def _(event):
    ...
    if len(input_sgra) == 3:
        ...
        await user.edit(str(a))
    else:
        await eod(user, f"SYNTAX: {hl}calendar YYYY.MM.DD")
    end = datetime.datetime.now()
    (end - start).seconds
...
async def _(event):
    if event.fwd_from:
        return
    userevent = await eor(event, "`processiong...`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
...
CmdHelp("tools").add_command(
  ...
  "currency", "<amount> <from> <to>", "Currency converter for Andencento ", ".currency 10 usd inr"
).add_command(
