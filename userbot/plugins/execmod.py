...
@Andencento.on(admin_cmd(pattern="pips(?: |$)(.*)", outgoing=True))
@Andencento.on(sudo_cmd(pattern="pips(?: |$)(.*)", allow_sudo=True))
...
@Andencento.on(sudo_cmd(pattern="suicide$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    o = stdout.decode()
    OUTPUT = f"**[ᴀɴᴅᴇɴᴄᴇɴᴛᴏ's](tg://need_update_for_some_feature/) SUICIDE BOMB:**\n{o}"
...
@Andencento.on(sudo_cmd(pattern="date$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    PROCESS_RUN_TIME = 100
...
@Andencento.on(sudo_cmd(pattern="env$", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    PROCESS_RUN_TIME = 100
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[ᴀɴᴅᴇɴᴄᴇɴᴛᴏ's](tg://need_update_for_some_feature/) Environment Module:**\n\n\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await bot.send_file(
...
@Andencento.on(admin_cmd(pattern="speed$"))
@Andencento.on(sudo_cmd(pattern="speed$", allow_sudo=True))
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[ᴀɴᴅᴇɴᴄᴇɴᴛᴏ's](tg://need_update_for_some_feature/) , Server Speed Calculated:**\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await bot.send_file(
